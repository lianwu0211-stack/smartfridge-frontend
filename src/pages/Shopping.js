...
export default function Shopping() {
  ...
  const [form, setForm]           = useState({ ingredient: "", quantity: "", unit: "pcs" });
  ...
  const handleAdd = async () => {
    ...
    try {
      await axios.post(`${API}/shopping`, {
        ...
        quantity: parseFloat(form.quantity), unit: form.unit || "pcs",
      }, { headers });
      setForm({ ingredient: "", quantity: "", unit: "pcs" }); setShowAdd(false); fetchItems();
  ...
  const handleConfirmPurchase = async () => {
    ...
    try {
      ...
      const existing = fridgeRes.data.find(i => i.ingredient === expiryModal.ingredient);
      if (existing) {
        // 冰箱已有同名食材 → 累加数量并更新保质期
        await axios.put(`${API}/fridge/${existing.item_id}`, {
          quantity: existing.quantity + expiryModal.quantity,
  ...
  const handleBatchPurchase = async () => {
    ...
    try {
      ...
      await Promise.all(pending.map(async (item) => {
        ...
        if (expiry) {
          const existing = fridgeItems.find(fi => fi.ingredient === item.ingredient);
          if (existing) {
            // 冰箱已有 → 累加数量
            await axios.put(`${API}/fridge/${existing.item_id}`, {
        ...
        <div style={styles.modalOverlay}>
          <div style={styles.modalCard}>
            ...
            <div style={styles.fieldGroup}>
              ...
              <input style={styles.select} type="date" value={expiryDate}
